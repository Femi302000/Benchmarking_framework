...
from tf_lookup import BagTfProcessor
...
if __name__ == "__main__":
    ...
    if VIS_OVERLAY:
        ...
        visualize_overlap_points(scene[0], str(model_path), distance_threshold=0.1)
    if VIS_OVERLAP and scene_files :
        for scene in scene_files :
            filtered_scene = str(scene)
            visualize_overlap_points(filtered_scene, str(model_path))

    if ALIGN_MODELS and scene_files:
        align_and_save_models(bag_name, scene_files, str(model_path), Path(BAG_PATH).parents[1])
    print("=== DONE ===")
