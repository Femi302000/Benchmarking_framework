...
# ---- Paths & Transform ----
source_path = "/home/femi/Evitado/Benchmarking_framework/Data/Aircraft_models/HAM_Airport_2024_08_08_movement_e175s_model.pcd"
target_folder = "/home/femi/Evitado/Benchmarking_framework/Data/sequence_from_scene/HAM_Airport_2024_08_08_movement_e175s"

label_dir = "/home/femi/Evitado/Benchmarking_framework/Data/Machine_learning_dataset/label"
scene_dir = "/home/femi/Evitado/Benchmarking_framework/Data/Machine_learning_dataset/scene"
...
for target_path in glob.glob(target_pattern):
    ...
    candidates = [f"{json_base}_transformation.json", f"{json_base}_tf.json", f"{json_base}_transformation.json", f"{json_base}.json"]
    json_path = None
    ...
    # Load, transform source model, compute bounding box
    pcd_src = o3d.io.read_point_cloud(source_path)
    pcd_src.transform(T)
    aabb = pcd_src.get_oriented_bounding_box()

    # Load target scene
    pcd_tgt = o3d.io.read_point_cloud(target_path)
    pts = np.asarray(pcd_tgt.points)

    # Label points within bounding box
    in_indices = aabb.get_point_indices_within_bounding_box(pcd_tgt.points)
